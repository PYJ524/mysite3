					...
					<table>
						...
						<c:forEach items="${rbList}" var="rBoardVo">
							<tbody>
								<tr>
									<td>${rBoardVo.no}</td>
									<td class="text-left"><c:forEach begin="1" end="${rBoardVo.depth}">
								        &nbsp;&nbsp;&nbsp;
								    </c:forEach> <a href="${pageContext.request.contextPath}/rBoard/read?no=${rBoardVo.no}">${rBoardVo.title} </a></td>
									<td>${rBoardVo.name}</td>
									<td>${rBoardVo.hit}</td>
